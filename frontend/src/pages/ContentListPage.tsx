...
const ContentListPage = () => {
  ...
  const slug = contentType || "articles"; // default to articles if undefined
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background to-accent">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground capitalize">{contentType?.replace("-", " ")}</h1>
          <p className="text-muted-foreground text-lg">Browse the latest content and resources.</p>
        </div>
        ...
        {stories.length > 0 ? (
              ...
              return (
                <Link to={`/articles/${storyItem.slug}`} key={storyItem.uuid}>
                  <div className="bg-card shadow-md rounded-lg overflow-hidden hover:shadow-xl transition-shadow duration-300 cursor-pointer">
                    {featuredImage && <img src={featuredImage} alt={storyItem.content.featured_image?.alt || ""} className="w-full h-48 object-cover" />}
                    <div className="p-4">
                      <h2 className="text-xl font-semibold mb-2">{storyItem.content.title}</h2>
                      {storyItem.content.subtitle && <p className="text-gray-500 mb-2">{storyItem.content.subtitle}</p>}
                      {storyItem.content.summary && <p className="text-gray-600 text-sm mb-2">{storyItem.content.summary}</p>}
                      {storyItem.content.tags?.length > 0 && (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-muted-foreground mt-2">There are no items to display in this section yet.</p>
          </div>
        )}
      </main>
    </div>
